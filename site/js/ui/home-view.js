import {renderCityScene} from '../city/renderer.js?v=0.7.1';
import {deriveCityVisualState} from '../city/visual-state.js';
import {derivePonkichiReaction,renderPonkichi} from '../characters/ponkichi.js?v=0.7.1';
...
const PHASE_GUIDES={
  focus:{eyebrow:'最初にやること',title:'今週の重点方針を決める',description:'市政の優先順位とポン吉の委任方針を選びます。ここから第1週が始まります。'},
...
export function renderHomeView(state,uiState={}){
  ...
  return `<main class="view home-view">
    <section class="week-heading"><div><span>第${state.week}週・第${state.term}期</span><h2>${PHASE_LABELS[state.phase]}</h2></div><button class="phase-status-button ${state.weeklyFocus?'is-set':''}" data-action="phase-action">${state.weeklyFocus?'方針設定済み':'方針未設定'}</button></section>
    <section class="next-action-card">
      <div class="next-action-character">${renderPonkichi(reaction,state.equippedSkin)}</div>
      <div class="next-action-copy"><span>${guide.eyebrow}</span><h3>${guide.title}</h3><p>${escapeHtml(reaction.line)}</p><small>${guide.description}</small></div>
